...
Процедура ТестДолжен_ПроверитьНазначениеПутиКПлатформе() Экспорт
	...
	ПутьПоУмолчанию = ПолучитьПутьКВерсииПлатформы("8.3");
	юТест.ПроверитьЛожь(ПустаяСтрока(ПутьПоУмолчанию));
	юТест.ПроверитьРавенство(ПутьПоУмолчанию, ПутьКПлатформе1С());
	
	НовыйПуть = "тратата";
	ПутьКПлатформе1С(НовыйПуть);
	юТест.ПроверитьРавенство(НовыйПуть, ПутьКПлатформе1С());
