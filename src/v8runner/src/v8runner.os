...
Процедура ЗагрузитьЭталон(ПутьЭталоннойВыгрузки) Экспорт
	ФайлВыгрузки = Новый Файл(ПутьЭталоннойВыгрузки);
	Ожидаем.Что(ФайлВыгрузки.Существует(), "Файл выгрузки <"+ПутьЭталоннойВыгрузки+"> существует, а это не так").ЭтоИстина();
	
	ПараметрыЗапуска = СтандартныеПараметрыЗапускаКонфигуратора();
	
	ПараметрыЗапуска.Добавить("/RestoreIB " + ОбернутьВКавычки(ПутьЭталоннойВыгрузки));
