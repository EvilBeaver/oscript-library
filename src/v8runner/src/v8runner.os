...
Процедура ЗагрузитьКонфигурациюИзФайла(Знач ФайлКонфигурации, Знач ОбновитьКонфигурациюИБ = Ложь, Знач ФайлСконвертирован = ЛОЖЬ) Экспорт
	...
	ТекстОшибки = ВыполнитьКоманду(Параметры, ИСТИНА); // для получения текста ошибки
	...
	Иначе
		...
		Если НЕ ФайлСконвертирован 
				И Найти(Врег(ТекстОшибки), Врег("Структура конфигурации несовместима с текущей версией программы")) Тогда
			
			// добавим проверку на наличие несовместимости версий,
			// требующих конвертацию конфигурации перед загрузкой		
			Параметры = СтандартныеПараметрыЗапускаКонфигуратора();
			Параметры.Добавить("/ConvertFiles""" + ФайлКонфигурации + """");
			ВыполнитьКоманду(Параметры);
			
			ЗагрузитьКонфигурациюИзФайла(ФайлКонфигурации, ОбновитьКонфигурациюИБ, ИСТИНА);
			Возврат;
...
Функция ВыполнитьКоманду(Знач Параметры, Знач ВозвращатьОшибку = ЛОЖЬ) Экспорт
	...
	УстановитьВывод(ПрочитатьФайлИнформации());
	Если КодВозврата <> 0 Тогда
		Если ВозвращатьОшибку Тогда
			Лог.Информация("Получен ненулевой код возврата "+КодВозврата+"!");		
