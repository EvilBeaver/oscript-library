...
Процедура ВыгрузитьКонфигурациюВФайлы(Знач КаталогВыгрузки, Знач ФорматВыгрузки = "Hierarchical") Экспорт
	
	Параметры = СтандартныеПараметрыЗапускаКонфигуратора();
	Параметры.Добавить("/DumpConfigToFiles " + ОбернутьВКавычки(КаталогВыгрузки) + " -format " + ФорматВыгрузки);
	ВыполнитьКоманду(Параметры);
