...
//
// Параметры:
//   ТипПервойКонфигурации - ТипКонфигурацииДляСравнения - тип первой конфигурации для сравнения.
//		Допустимые значения содержатся в перечислении "ТипКонфигурацииДляСравнения"
//   ДополнительныйИдентификаторПервойКонфигурации - Строка - дополнительный идентификатор первой конфигурации.
//		Может принимать значения:
//			| Тип конфигурации | Значение идентификатора |
//			| ОсновнаяКонфигурация					| Не используется 										|
//			| КонфигурацияБазыДанных				| Не используется 										|
//			| КонфигурацияПоставщика				| Имя конфигурации поставщика 							|
//			| РасширениеКонфигурации				| Имя конфигурации расширения 							|
//			| РасширениеКонфигурацииИзБазыДанных	| Имя конфигурации расширения (из базы данных) 			|
//			| КонфигурацияИзХранилищаКонфигурации	| Версия конфигурации в хранилище 						|
//			| Файл									| Путь к файлу конфигурации или расширения конфигурации |
//   ТипВторойКонфигурации - ТипКонфигурацииДляСравнения - тип второй конфигурации для сравнения.
//		Допустимые значения содержатся в перечислении "ТипКонфигурацииДляСравнения"
//   ДополнительныйИдентификаторВторойКонфигурации - Строка - дополнительный идентификатор второй конфигурации.
//		Может принимать значения:
//			| Тип конфигурации | Значение идентификатора |
//			| ОсновнаяКонфигурация					| Не используется 										|
//			| КонфигурацияБазыДанных				| Не используется 										|
...
//			| КонфигурацияИзХранилищаКонфигурации	| Версия конфигурации в хранилище 						|
//			| Файл									| Путь к файлу конфигурации или расширения конфигурации |
//   ТипОтчетаОСравнении - ТипОтчетаОСравнении - тип отчета о сравнении.
//		Допустимые значения содержатся в перечислении "ТипОтчетаОСравнении"
//   ФорматФайлаОтчетаОСравнении - ФорматФайлаОтчетаОСравнении - формат файла отчета.
//		Допустимые значения содержатся в перечислении "ФорматФайлаОтчетаОСравнении"
//   ФайлОтчетаОСравнении - Строка - имя файла, в который будет помещен отчет о сравнении.
//   ПравилоУстановкиСоответствийОбъектов - ПравилоУстановкиСоответствийОбъектов - правило установки соответствий
//		объектов, для тех случаев, когда конфигурации не состоят в отношениях "родитель-потомок".
//		Допустимые значения содержатся в перечислении "ПравилоУстановкиСоответствийОбъектов"
//   ФайлСписокОбъектов - Строка - путь к файлу со списком объектов, которые будут участвовать в операции.
//		Если файл указан – в операции участвуют только указанные в файле объекты, в противном случае участвует
//		вся конфигурация.
//   ВключатьВОтчетИзмененныеОбъекты - Булево - включать в отчет измененные подчиненные объекты.
//   ВключатьВОтчетУдаленныеОбъекты - Булево - включать в отчет удаленные подчиненные объекты.
//   ВключатьВОтчетДобавленныеОбъекты - Булево - включать в отчет добавленные подчиненные объекты.
//
Процедура СравнитьКонфигурации(Знач ТипПервойКонфигурации,
							   Знач ДополнительныйИдентификаторПервойКонфигурации = "",
							   Знач ТипВторойКонфигурации,
							   Знач ДополнительныйИдентификаторВторойКонфигурации = "",
							   Знач ТипОтчетаОСравнении,
							   Знач ФорматФайлаОтчетаОСравнении,
							   Знач ФайлОтчетаОСравнении,
							   Знач ПравилоУстановкиСоответствийОбъектов = "",
							   Знач ФайлСписокОбъектов = "",
							   Знач ВключатьВОтчетИзмененныеОбъекты = Ложь,
							   Знач ВключатьВОтчетУдаленныеОбъекты = Ложь,
							   Знач ВключатьВОтчетДобавленныеОбъекты = Ложь) Экспорт

	Параметры = СтандартныеПараметрыЗапускаКонфигуратора();
	Параметры.Добавить("/CompareCfg");

	Параметры.Добавить("–FirstConfigurationType " + ТипПервойКонфигурации);
	Если ЗначениеЗаполнено(ДополнительныйИдентификаторПервойКонфигурации) Тогда
		Параметры.Добавить("-FirstConfigurationKey " + ДополнительныйИдентификаторПервойКонфигурации);
	КонецЕсли;

	Параметры.Добавить("–SecondConfigurationType " + ТипВторойКонфигурации);
	Если ЗначениеЗаполнено(ДополнительныйИдентификаторПервойКонфигурации) Тогда
		Параметры.Добавить("-SecondConfigurationKey " + ДополнительныйИдентификаторВторойКонфигурации);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПравилоУстановкиСоответствийОбъектов) Тогда
		Параметры.Добавить("-MappingRule " + ПравилоУстановкиСоответствийОбъектов);
	КонецЕсли;

	Если ЗначениеЗаполнено(ФайлСписокОбъектов) Тогда
		Параметры.Добавить("-Objects " + ОбернутьВКавычки(ФайлСписокОбъектов));
	КонецЕсли;

	Параметры.Добавить("–ReportType " + ТипОтчетаОСравнении);
	
	Если ВключатьВОтчетИзмененныеОбъекты Тогда
		Параметры.Добавить("-IncludeChangedObjects");
	КонецЕсли;
	Если ВключатьВОтчетУдаленныеОбъекты Тогда
		Параметры.Добавить("-IncludeDeletedObjects");
	КонецЕсли;
	Если ВключатьВОтчетДобавленныеОбъекты Тогда
		Параметры.Добавить("-IncludeAddedObjects");
	КонецЕсли;

	Параметры.Добавить("-ReportFormat " + ФорматФайлаОтчетаОСравнении);
	Параметры.Добавить("-ReportFile " + ОбернутьВКавычки(ФайлОтчетаОСравнении));
