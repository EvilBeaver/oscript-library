...
//   - 1 - создавать объекты (по умолчанию)
//   - 2 - очищать объекты
//   - 3 - не изменять при частичной потере объектов
//
...
Функция ПолучитьОпцииТиИ() Экспорт
	...
	СтруктураОпцииТиИ.Вставить("ПересчеИтогов", Истина);
	СтруктураОпцииТиИ.Вставить("СжатиеТаблиц", Истина);
	СтруктураОпцииТиИ.Вставить("Реструктуризация", Истина);
	СтруктураОпцииТиИ.Вставить("ВариантОбработкиНесуществующихСсылок", 1);
	СтруктураОпцииТиИ.Вставить("ВариантОбработкиЧастичнойПотериДанных", 1);
...
//   // Отключили проверку логической и ссылочной целостности
...
Процедура ЗапуститьТестированиеИИсправление(Параметры = Неопределено) Экспорт
	...
	СтрокаОпций = СтрокаОпций + ?(Параметры.ПересчеИтогов, " -RecalcTotals", "");
	...
	СтрокаОпций = СтрокаОпций + ?(БитыеСсылки = 1, " -BadRefCreate", ?(БитыеСсылки = 2, " -BadRefClear", " -BadRefNone"));

	ЧастичнаяПотеря = Параметры.ВариантОбработкиЧастичнойПотериДанных;
	СтрокаОпций = СтрокаОпций + ?(ЧастичнаяПотеря = 1, " -BadDataCreate", " -BadDataDelete");
