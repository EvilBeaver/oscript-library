...
Процедура ДобавитьОписаниеМетаданныхПакета(Знач Архив, Знач Манифест);
	...
	Архив.Добавить(ПутьМанифеста, РежимСохраненияПутейZIP.СохранятьОтносительныеПути);
...
Процедура ДобавитьОписаниеБиблиотеки(Знач Архив, Знач Манифест)
	...
	Архив.Добавить(ПутьКонфигурацииПакета, РежимСохраненияПутейZIP.СохранятьОтносительныеПути);
...
Процедура ЗаписатьСвойстваПакета(Знач Запись, Знач Манифест)
	...
	Если Не ЗначениеЗаполнено(СвойстваПакета.Имя) Тогда
		ВызватьИсключение "Не задано имя пакета";
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СвойстваПакета.Версия) Тогда
