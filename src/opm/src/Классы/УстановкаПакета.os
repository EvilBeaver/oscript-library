...
Процедура УстановитьПакетыПоЗависимостям() Экспорт

	ПутьКФайлуКонфиг = ОбъединитьПути(ТекущийКаталог(), "oscript.cfg");
	ФайлКонфиг = Новый Файл(ПутьКФайлуКонфиг);
	УстанавливаемаяСтрокаКонфига = "lib.system = ./oscript_modules";
	Если НЕ ФайлКонфиг.Существует() Тогда
		ТекстКонфига = УстанавливаемаяСтрокаКонфига;
	Иначе
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ПутьКФайлуКонфиг);

		ТекстКонфига = ТекстовыйДокумент.ПолучитьТекст();
		Лог.Отладка("Старый конфиг: " + Символы.ПС + ТекстКонфига);

		// Находим последнюю строку с настройкой
		КоличествоСтрок = ТекстовыйДокумент.КоличествоСтрок();
		Для сч = 0 По КоличествоСтрок - 1 Цикл

			ИндексСтроки = КоличествоСтрок - сч;
			Строка = ТекстовыйДокумент.ПолучитьСтроку(ИндексСтроки);
			ЭтоСтрокаСистемнойНастройки = СтрНайти(Строка, "lib.system") > 0 И Лев(СокрЛП(Строка), 1) <> "#";
			Если ЭтоСтрокаСистемнойНастройки Тогда
				ТекстовыйДокумент.ЗаменитьСтроку(ИндексСтроки, УстанавливаемаяСтрокаКонфига);
				Прервать;
			КонецЕсли;
	...
	Лог.Отладка("Путь к файлу конфига" + ПутьКФайлуКонфиг);
	Лог.Отладка("Новый конфиг: " + Символы.ПС + ТекстКонфига);

	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.УстановитьТекст(ТекстКонфига);
	ТекстовыйДокумент.Записать(ПутьКФайлуКонфиг, КодировкаТекста.UTF8NoBOM);
