...
Процедура УстановитьПакетИзОблака(Знач ОписаниеПакета)
	...
	ФайлПакета = ОписаниеПакета.ИмяПакета + "-" + ОписаниеПакета.Версия + ".ospx";
	
	Сервер = "http://hub.oscript.io";
	Ресурс = "/download/" + ОписаниеПакета.ИмяПакета + "/" + ФайлПакета;
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		КонецПопытки;
	Иначе
		ВызватьИсключение "Ошибка установки пакета " + ОписаниеПакета.ИмяПакета + "
		|" + Ответ.КодСостояния;
	КонецЕсли;
