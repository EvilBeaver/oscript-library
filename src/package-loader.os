...
Процедура ПриЗагрузкеБиблиотеки(Путь, СтандартнаяОбработка, Отказ)
	...
	ФайлИнициализации = Новый Файл(ОбъединитьПути(Путь, "__init__.os"));
	
	Если ФайлМанифеста.Существует() Тогда
		СтандартнаяОбработка = Ложь;
		ОбработатьМанифест(ФайлМанифеста.ПолноеИмя, Путь, Отказ);
	ИначеЕсли ФайлИнициализации.Существует() Тогда
		ВнешнийОбработчик = ЗагрузитьСценарий(ФайлИнициализации.ПолноеИмя);
		ВнешнийОбработчик.ПриЗагрузкеБиблиотеки(ЭтотОбъект, Путь, СтандартнаяОбработка, Отказ);
	КонецЕсли;
