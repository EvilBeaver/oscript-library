...
Процедура ДобавитьАргументыПоУмолчанию(Знач Парсер)
	...
	Парсер.ДобавитьИменованныйПараметр("-gitpublish", "<on|off> получать/отправлять с удаленных хранилищ");
...
Процедура Синхронизировать(Знач ПутьКХранилищу, 
			...
			Знач ВыполнятьPullPush = Неопределено) Экспорт
	...
	Если ВыполнятьPullPush = Неопределено Тогда
		ВыполнятьPullPush = "on";
	КонецЕсли;
	
	ОбновлятьУдаленныеРепозитории=?(ВыполнятьPullPush="on", Истина, Ложь);
	...
	Лог.Отладка("ВыполнятьPullPush = " + ВыполнятьPullPush);
	...
	Если ОбновлятьУдаленныеРепозитории Тогда
