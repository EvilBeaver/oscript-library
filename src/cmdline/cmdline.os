...
Функция РазобратьКоманду(Знач МассивПараметров) Экспорт
	...
	Для Сч = ОписаниеКоманды.ПозиционныеПараметры.Количество()+1 По МассивПараметров.ВГраница() Цикл
...
Функция РазобратьИменованныеПараметры(Знач ОписаниеПараметров, Знач ВходнойМассивПараметров)
	...
	Для Сч=0 по ВходнойМассивПараметров.ВГраница() Цикл
		...
		Если КлючЗначение = Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		Значение = ?(Сч < ВходнойМассивПараметров.ВГраница(), ВходнойМассивПараметров[Сч+1], Истина);
